...
dynamic SBS_RKHS::compute_dynamics(const Eigen::MatrixXd &particles, const function<double(dyn_vector x)> &f, vector<double> *evals, const int &time)
{
  ...
  Eigen::MatrixXd noise = this->compute_noise(particles);
