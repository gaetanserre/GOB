...
REAL f_bobyqa(const INTEGER n, const REAL *x, void *data)
{
  dyn_vector x_dyn(n);
  for (int i = 0; i < n; i++)
  {
  ...
  function<double(dyn_vector x)> *f = (function<double(dyn_vector x)> *)data;
  return (*f)(x_dyn);
}

result_eigen run_bobyqa(const vec_bounds bounds, const dyn_vector x_dyn, const INTEGER maxfun, function<double(dyn_vector x)> *f)
{
  const int n = bounds.size();
  const int npt = n + 2;
  REAL *x = (double *)x_dyn.data();
  REAL xl[n];
  REAL xu[n];
  for (int i = 0; i < n; i++)
  {
  ...
  const REAL rhobeg = 1e-3;
  const REAL rhoend = 1e-8;
  const INTEGER iprint = 0;
  int nw = (npt + 5) * (npt + n) + 3 * n * (n + 5) / 2;
  REAL work[nw];

  int res = bobyqa(n, npt, f_bobyqa, (void *)f, x, xl, xu, rhobeg, rhoend, iprint, maxfun, work);
  dyn_vector x_opt(n);
  for (int i = 0; i < n; i++)
  {
    x_opt[i] = x[i];
  }
  return make_pair(x_opt, (*f)(x_opt));
}